...
export default {
  clickPl(state, {record}){

    const {selectedPls} = state
    const isContains = selectedPls.some(spl=>spl.id == record.id)

    if (isContains) {
      return {
        ...state,
        selectedPls: selectedPls.filter(pl=>pl.id != record.id)
      }
    } else {
      return {
        ...state,
        selectedPls: [...selectedPls, record]
      }
    }
  },
  menuNav(state, {data, first, pageName}){
  ...
  showPbDel(state, {pbs}){
    const newComps = [...pbs.map(pb=>F.genPbKey(pb)), C.SUBMIT_BTN_KEY]
    const comps = {
      ...state.comps,
      [C.MAIN_BLOCK]: newComps
    }
