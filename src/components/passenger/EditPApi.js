 ...
 *	
 */
class EditPApi extends React.Component {
		 ...
		 this.props.form.validateFieldsAndScroll((err, values) => {
		      if (!err) {
					let params = {
					     ...
					     uniqueId:that.state.uui             // 旅客uui
					}
					that.props.dispatch({type:"dcs/editPApi" , payload:params});
		      }
	    });
	}
					...
					<Col span={12}>
						<FormItem label="旅客姓氏:" {...formItemLayout}>
							{getFieldDecorator('lastname', {
					            rules: [{ required: true, message: '请输入旅客姓氏!'}],
					            initialValue:detail.dln
					          })(
					...
					<Col span={12}>
						<FormItem label="旅客名字:" {...formItemLayout}>
							{getFieldDecorator('firstname', {
					            rules: [{ required: true, message: '请输入旅客名字!'}],
					            initialValue:detail.cna
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="出生日期:" {...formItemLayout}>
							{getFieldDecorator('birth', {
					            ...
					            initialValue: detail.db && moment(detail.db, 'YYYY-MM-DD')
					          })(
					            <DatePicker />
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="旅客国籍:" {...formItemLayout}>
							{getFieldDecorator('nationality', {
					            rules: [{ required: true, message: '请输入旅客国籍!'}],
					            initialValue:detail.dc
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="证件签发国:" {...formItemLayout}>
							{getFieldDecorator('passportCountry', {
					            rules: [{ required: true, message: '请输入证件签发国!'}],
					            initialValue:detail.di
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="ID-Type:" {...formItemLayout}>
							{getFieldDecorator('idtype', {
					            rules: [{ required: true, message: '请输入ID类型!'}],
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="证件失效日期:" {...formItemLayout}>
							{getFieldDecorator('expiryDate', {
					            ...
					            initialValue: detail.de && moment(detail.de, 'YYYY-MM-DD')
					          })(
					             <DatePicker />
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="客票类型:" {...formItemLayout}>
							{getFieldDecorator('ticket-type', {
					            rules: [{ required: true, message: '请输入客票类型!'}],
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
					...
					<Col span={12}>
						<FormItem label="客票号码:" {...formItemLayout}>
							{getFieldDecorator('ticketNumber', {
					            rules: [{ required: true, message: '请输入客票号码!'}],
					          })(
					            <Input placeholder="请输入内容"/>
					         )}
						</FormItem>
...
export default Form.create()(EditPApi)
